...
public sealed class MemoryTools
{
    ...
    [Description("Scan notes for data-quality issues (no_tags, no_dedup_key, no_title, duplicate, broken_link, stale_unstructured, possible_secret) and return findings (noteId/title/domain/type/rule/severity/message). 'duplicate' = another active note shares the same domain/type/title; 'stale_unstructured' = tagged 'unstructured' and untouched for 30+ days (review queue); 'possible_secret' = body/payload heuristically looks like it embeds a credential (the secret value is never echoed). Pass a domain to focus. Read-only — it suggests fixes, changes nothing.")]
