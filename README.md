...
A thin, reliable data-plane [MCP](https://modelcontextprotocol.io) server for personal memory, notes and skills — shared by multiple agents (a Home Assistant agent, Claude Code, a future web UI) over the Model Context Protocol. The server only **stores, validates, searches and returns**; all the intelligence (what to write, how to phrase it) lives in the agents and their skills.
