...
- **Auth with domain scoping is mandatory**, not "later". Bearer token + an allowed-domains list per token. **Discovery is scoped like retrieval** (MEMP-232): a facet, domain or count listing must pass the caller's `ReadRestriction` too. A discovery surface wider than the retrieval surface both leaks the shape of other domains and hands agents references they cannot resolve — the guard test in `ReadToolScopeGuardTests` enforces this across the whole read surface.
